/*  NASA Space Apps Challenge 2021
	AirSecure: Mutation-based Constraint Optimizer
...
const float MUTATION_PROBABILITY = 0.5;
...
// to initialize population with random values
...
// to evaluate the chromosomes by calculating fitness
...
// to store the best performing chromosome according to fitness value
...
// to calculate average fitness value of a population
void calcAvgFitness() {
	double sum = 0.0;
	for (int c = 0;c < POP_SIZE;c++) {
		sum += fitness[c];
...
// to select parent chromosome using tournament selection 
...
//to perform crossover with parent chromosomes using 3-point crossover
void crossover() {
	cout << "\n\nCROSSOVER:\n";
	float randNo;
	int xpoint;
...
// to perform mutation on chidren chromosomes using random mutation which mutates three continous genes
void mutation() {
	cout << "\n\nMUTATION:\n";
	float randVal;
...
// to select survivors from children chromosomes
...
// to copy survivor chromosomes to current population
void copyChromosome() {
	for (int i = 0;i < POP_SIZE;i++) {
		for (int j = 0;j < GENE;j++) {
...
// to print genetic algorithm results
void printResult() {
	cout << "RESULTS:\n";
...
// to generate graph of best fitness and average fitness across population
void generateGraph() {
